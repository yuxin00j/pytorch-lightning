...
- Optimized remote checkpoint loading by delegating to `fs.get_file` with a node-local file lock (`fcntl.flock`), a version-keyed cache and `mmap=True` in `_load`, configurable through `LIGHTNING_CHECKPOINT_CACHE` and `LIGHTNING_CHECKPOINT_CACHE_DIR` ([#21869](https://github.com/Lightning-AI/pytorch-lightning/pull/21869))
