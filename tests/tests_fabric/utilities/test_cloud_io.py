...
def _mp_worker(local_rank, ckpt_path_str, cache_root_str, counter_dir_str, size, barrier):
...
def test_load_remote_multiprocess_singleflight(tmp_path):
    """Multiple processes (LOCAL_RANK=0..3) loading the same remote checkpoint concurrently download it once."""
    ...
    procs = [
        ctx.Process(target=_mp_worker, args=(rank, str(ckpt_path), str(cache_root), str(counter_dir), size, barrier))
...
def test_load_remote_peer_rank_error_propagation_and_timeout(tmp_path, monkeypatch):
    """Non-zero local ranks must surface local_rank=0 download errors immediately and time out if nothing arrives."""
    ckpt_path = tmp_path / "peer_err.ckpt"
    size = _big_checkpoint(ckpt_path)
    ...
    # 1. Simulate local_rank=0 failing during get_file, then local_rank=1 attempting to load
    class FailingRank0FS:
        def info(self, path):
            return {"size": size, "etag": "v-err"}

        def get_file(self, rpath, lpath):
            raise RuntimeError("gcs 503 service unavailable")

    monkeypatch.setattr("lightning.fabric.utilities.cloud_io.get_filesystem", lambda _: FailingRank0FS())
    monkeypatch.setenv("LOCAL_RANK", "0")
    with pytest.raises(RuntimeError, match="gcs 503 service unavailable"):
        _load(str(ckpt_path), map_location="cpu")

    monkeypatch.setenv("LOCAL_RANK", "1")
    with pytest.raises(RuntimeError, match="local_rank=0 failed to download.*gcs 503 service unavailable"):
        _load(str(ckpt_path), map_location="cpu")

    # 2. Simulate local_rank=1 timing out when local_rank=0 never writes the checkpoint or error marker
    clear_cache()
    monkeypatch.setattr("lightning.fabric.utilities.cloud_io._CACHE_WAIT_TIMEOUT_SECONDS", 0.1)
    monkeypatch.setattr("lightning.fabric.utilities.cloud_io._CACHE_POLL_INTERVAL_SECONDS", 0.02)
    with pytest.raises(TimeoutError, match="Timed out.*waiting for local_rank=0"):
        _load(str(ckpt_path), map_location="cpu")
